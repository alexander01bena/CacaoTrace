...
class Productor(db.Model):
    ...
    municipio = db.Column(db.String(80), default="San Andrés de Tumaco")
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    fincas = db.relationship("Finca", back_populates="productor", cascade="all, delete-orphan")
...
class Lote(db.Model):
    ...
    controles_calidad = db.relationship("ControlCalidad", back_populates="lote", cascade="all, delete-orphan")
    procesos_poscosecha = db.relationship("Poscosecha", back_populates="lote", cascade="all, delete-orphan")
    movimientos = db.relationship("MovimientoInventario", back_populates="lote", cascade="all, delete-orphan")
...
class MovimientoInventario(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    tipo = db.Column(db.String(30), nullable=False)  # Entrada, Salida o Ajuste
    cantidad_kg = db.Column(db.Float, nullable=False)
