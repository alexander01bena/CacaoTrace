...
def recalcular_cantidad_lote(lote):
    ...
    if lote.cantidad_actual_kg == 0:
        lote.estado = "Agotado"
    elif lote.estado == "Agotado" and lote.cantidad_actual_kg > 0:
        lote.estado = "Inventario"
...
def productores():
    if request.method == "POST":
        ...
        municipio = request.form.get("municipio", "San Andrés de Tumaco").strip()

        if not nombres:
            flash("El nombre del productor es obligatorio.", "warning")
            return redirect(url_for("main.productores"))
...
def editar_productor(productor_id):
    ...
    if request.method == "POST":
        ...
        productor.municipio = request.form.get("municipio", "San Andrés de Tumaco").strip()

        if not productor.nombres:
            flash("El nombre del productor no puede quedar vacío.", "warning")
            return redirect(url_for("main.editar_productor", productor_id=productor.id))
...
def fincas():
    ...
    if request.method == "POST":
        ...
        if not nombre or not productor_id:
            ...
            return redirect(url_for("main.fincas"))
...
def editar_finca(finca_id):
    ...
    if request.method == "POST":
        ...
        if not nombre or not productor_id:
            flash("El nombre de la finca y el productor son obligatorios.", "warning")
            return redirect(url_for("main.editar_finca", finca_id=finca.id))
...
def recepciones():
    ...
    if request.method == "POST":
        ...
        if not productor_id or not finca_id or not tipo_cacao or not cantidad_kg:
            ...
            return redirect(url_for("main.recepciones"))
        ...
        return redirect(url_for("main.detalle_lote", lote_id=lote.id))
...
def editar_recepcion(recepcion_id):
    ...
    if request.method == "POST":
        ...
        if not productor_id or not finca_id or not tipo_cacao or not cantidad_kg:
            ...
            return redirect(url_for("main.editar_recepcion", recepcion_id=recepcion.id))
...
def editar_lote(lote_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            flash(str(error), "warning")
            return redirect(url_for("main.editar_lote", lote_id=lote.id))
...
def registrar_calidad(lote_id):
    ...
    if request.method == "POST":
        ...
        if not humedad or not estado_grano or not clasificacion:
            ...
            return redirect(url_for("main.registrar_calidad", lote_id=lote.id))
        ...
        lote.estado = "Calidad registrada"
...
def editar_calidad(control_id):
    ...
    if request.method == "POST":
        ...
        if not humedad or not estado_grano or not clasificacion:
            flash("Humedad, estado del grano y clasificación son obligatorios.", "warning")
            return redirect(url_for("main.editar_calidad", control_id=control.id))
...
def eliminar_calidad(control_id):
    ...
    return redirect(url_for("main.detalle_lote", lote_id=lote_id))
...
def registrar_poscosecha(lote_id):
    ...
    if request.method == "POST":
        ...
        if not fermentacion or not secado or not estado_proceso:
            ...
            return redirect(url_for("main.registrar_poscosecha", lote_id=lote.id))
...
def editar_poscosecha(proceso_id):
    ...
    if request.method == "POST":
        ...
        if not fermentacion or not secado or not estado_proceso:
            flash("Fermentación, secado y estado del proceso son obligatorios.", "warning")
            return redirect(url_for("main.editar_poscosecha", proceso_id=proceso.id))
...
def inventario():
    if request.method == "POST":
        ...
        if tipo == "Salida" and cantidad > lote.cantidad_actual_kg:
        ...
        if tipo in ["Entrada", "Ajuste"] and lote.cantidad_actual_kg > 0:
            lote.estado = "Inventario"
...
def editar_movimiento(movimiento_id):
    ...
    if request.method == "POST":
        ...
        if tipo not in ["Entrada", "Salida", "Ajuste"]:
            flash("Tipo de movimiento no válido.", "danger")
            return redirect(url_for("main.editar_movimiento", movimiento_id=movimiento.id))
